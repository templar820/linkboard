...
| Источник | Маршрутизатор Nest: запрошенного пути не существует ни в одном контроллере |

Когда возникает: обращение к несуществующему `/api/*`-маршруту (опечатка в пути, устаревший клиент, сканер).
Отличается от `LINK_NOT_FOUND` тем, что не найден **маршрут**, а не доменный объект.

Введён по решению координатора 11.08.2026: до этого такие ответы приходили с кодом `INTERNAL_ERROR` при
HTTP-статусе 404, что нарушало правило «`INTERNAL_ERROR` ⇒ 500» и не позволяло фронтенду отличить отсутствующую
страницу от сбоя сервера. Вариант с `405 Method Not Allowed` рассмотрен и отклонён: 405 означает существующий
ресурс с неприменимым методом и по RFC 9110 обязан нести заголовок `Allow`, которого у несуществующего маршрута
быть не может.

Пример тела ответа:

```json
{ "data": null, "error": { "code": "NOT_FOUND", "message": "Route GET /api/unknown was not found" } }
```

Что показывает фронтенд: `ErrorState` «Раздел не найден». Возникновение этой ошибки в admin-panel — почти всегда
признак рассинхрона клиента с API, поэтому её стоит логировать отдельно от сетевых сбоев.
