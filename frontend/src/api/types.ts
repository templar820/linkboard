/**
 * Linkboard API — типы, СИНХРОНИЗИРОВАННЫЕ с `docs/api/types.ts`.
 *
 * ЭТОТ ФАЙЛ — НЕ ИСТОЧНИК ПРАВДЫ. Единый источник правды — `docs/api/types.ts`
 * (плюс `docs/api/contract.md`, `docs/api/error-codes.md`, `docs/api/openapi.yaml`)
 * в корне репозитория. Копия лежит здесь, а не импортируется напрямую, потому
 * что при сборке Docker-образа frontend в контекст попадает только директория
 * `./frontend` (см. `frontend/Dockerfile`, `COPY . .` после `deps`) — путь
 * `../docs/api/types.ts` вышел бы за границу build-контекста.
 *
 * ПРАВИЛО СИНХРОНИЗАЦИИ: файл нельзя менять в одностороннем порядке. Любое
 * изменение контракта проходит через `docs/api/types.ts` (единственный источник
 * правды, потребляемый также `api-tests`), и уже оттуда переносится сюда 1:1.
 * После фриза контракта (SYNC-1, см. `sprints/11-08/sprint-plan.md`) — только
 * через явное решение координатора спринта.
 */
